...
def process_function(models, environment, epochs):
    warm_up_train(models, environment, epochs)  # Adjust if run_sim expects different parameters
...
# Using a pool of workers equal to the number of cores, 4 cores
    with Pool(4) as pool:
        ...
        for _ in range(4):
             pool.apply_async(process_function, args=(models, environment, epochs))
