...
def train(models, environment, epochs):
    ...
    WARM_UP_EPOCHS = ConfigManager.get('WARM_UP_EPOCHS')
    RL_DURATION = ConfigManager.get('RL_DURATION')

    for epoch in range(WARM_UP_EPOCHS):
        state, network = environment.reset()
        while not done:
            for step in tqdm(range(int(RL_STEP_LENGTH)), desc="WarmUP"): # 2.5mins, 10 steps
                environment.simulator.system_time += TIME_STEP
                environment.simulator.run_cycle() # Run one cycle(15s)
            environment.warm_up_step()
