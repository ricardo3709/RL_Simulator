...
def feature_preparation(state):
    ...
    for idx, counts in enumerate(working_veh_nodes):
        node_id = idx + 1 
        area_id = node_lookup_table[node_lookup_table['node_id'] == node_id]['zone_id'].values[0]
        indice = np.where(working_veh_nodes[:,0] == area_id)[0]
        working_veh_nodes[indice,1] += counts
    
    for idx, counts in enumerate(request_gen_counts):
        node_id = idx + 1 
        area_id = node_lookup_table[node_lookup_table['node_id'] == node_id]['zone_id'].values[0]
        indice = np.where(request_gen_counts[:,0] == area_id)[0]
        request_gen_counts[indice,1] += counts
    
    for idx, counts in enumerate(request_rej_counts):
        node_id = idx + 1 
        area_id = node_lookup_table[node_lookup_table['node_id'] == node_id]['zone_id'].values[0]
        indice = np.where(request_rej_counts[:,0] == area_id)[0]
        request_rej_counts[indice,1] += counts
    
    for idx, counts in enumerate(request_attraction_counts):
        node_id = idx + 1 
        area_id = node_lookup_table[node_lookup_table['node_id'] == node_id]['zone_id'].values[0]
        indice = np.where(request_attraction_counts[:,0] == area_id)[0]
        request_attraction_counts[indice,1] += counts
