...
def train(models, environment, epochs):
    ...
    for epoch in range(epochs):
        ...
        while not done:
            for time_step in tqdm(range(int(RL_STEP_LENGTH))): # 10 mins, 40 steps
                environment.simulator.system_time += time_step * TIME_STEP
                environment.simulator.run_cycle() # Run one cycle(15s)
                if time_step == RL_STEP_LENGTH - 1: #last step
                    environment.simulator.create_report()
