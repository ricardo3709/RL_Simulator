...
class ManhattanTrafficEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.n_steps_delay = self.config.get("RL_STEP_LENGTH") # delay for one whole simulation
    ...
    def calculate_reward(self, past_rejections):
        RL_STEP_LENGTH = self.config.get("RL_STEP_LENGTH")
        reward = 0.0
        old_avg_rej = np.mean(past_rejections)
        current_avg_rej = np.mean(past_rejections[-RL_STEP_LENGTH:])
        ...
        reward = old_avg_rej - current_avg_rej
        return reward
