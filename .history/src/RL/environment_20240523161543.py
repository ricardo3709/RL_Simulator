...
class ManhattanTrafficEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.n_steps_delay = 2 # 2steps delay, one step is 5mins
    ...
    def step(self, action):
        ...
        if len(self.past_rejections) == self.n_steps_delay: # delay is over
            ...
            reward = 0.000001  # minimum reward until the delay is over
    ...
    def calculate_reward(self, past_rejections):
        reward = 0.0
        old_avg_rej = 0.0
        current_rej = past_rejections[-1]
        # Calculate reward based on past rejections
        for i in range(len(past_rejections)-1): 
            old_avg_rej += (1-past_rejections[i]) * (self.decay_factor ** i) 
        reward = (1-past_rejections[-1]) - old_avg_rej
        return reward
