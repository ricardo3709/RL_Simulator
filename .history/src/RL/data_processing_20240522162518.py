...
def feature_preparation(state):
    ...
    request_gen_counts = [sum(request_gen_deque[i]) for i in range(1,len(request_gen_deque))]
    request_rej_counts = [sum(request_rej_deque[i]) for i in range(1,len(request_rej_deque))]
